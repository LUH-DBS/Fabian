...
def group_uris(
    candidates: Dict[str, Tuple[Set[int], Set[int]]], res_filter
) -> List[Tuple[str, List[str]]]:
    """Groups the given candidates into "WebResources". Grouping is based on common uris and 
    the individual candidates' matches (decision depends on 'res_filter').

    Args:
        candidates (Dict[str, Tuple[Set[int], Set[int]]]): A uri -> matches mapping (candidates)
        res_filter ([type]): A filter that can be applied to bfs_filter.

    Returns:
        List[Tuple[str, List[str]]]: A list of resources, each represented by a path and all uris included in the resource.
    """
    forest = URITree.create_uri_trees(candidates)

    groups = []
    # filter_forest is used for visualization, has no other use.
    filtered_forest = []
    for tree in forest.values():
        filter_result = tree.bfs_filter(res_filter.node_filter)
...
class URITree:
    ...
    @staticmethod
    def create_uri_trees(
        candidates: Dict[str, Tuple[Set[int], Set[int]]]
    ) -> Dict[str, URITree]:
    ...
    def add_child(self, label: str) -> URITree:
    ...
    def bfs_filter(self, filter_func) -> List[URITree]:
