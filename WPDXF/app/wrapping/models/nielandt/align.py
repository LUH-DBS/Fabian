...
import numpy as np
from wrapping.models.nielandt.utils import (EDIT_ACTIONS, backtrack,
                                            edit_distances)
...
def align(xpaths: List[XPath]) -> List[XPath]:
    ...
    for i, xpath0 in enumerate(xpaths):
        for xpath1 in xpaths[i + 1 :]:
            edit_matrix = edit_distances(xpath0, xpath1)
            if edit_matrix[-1, -1] < min_cost[0][-1, -1]:
                min_cost = (edit_matrix, xpath0, xpath1)
    edit_matrix, xpath0, xpath1 = min_cost

    aligned_xpaths = align_new_path(backtrack(edit_matrix), xpath0, [xpath1])
    ...
    while xpaths:
        min_cost = (np.array([[np.inf]]),)
        for xpath0 in enumerate(xpaths):
            xpath_mean = []
            ...
            if mean_cost < min_cost[0]:
                min_cost = (mean_cost, edit_matrix, xpath0)
        _, edit_matrix, xpath0 = min_cost
        xpaths.remove(xpath0)
        aligned_xpaths = align_new_path(backtrack(edit_matrix), xpath0, aligned_xpaths)
...
def align_new_path(
    ...
    for step_idx0, step_idx1, action in actions:
        if action is EDIT_ACTIONS.INSERT_0:
            xpath_0.insert(off_0 + step_idx0, XPathNode(axisname="self"))
            off_0 += 1
        elif action is EDIT_ACTIONS.INSERT_1:
            for xpath in xpaths:
                xpath.insert(off_1 + step_idx1, XPathNode(axisname="self"))
            off_1 += 1
